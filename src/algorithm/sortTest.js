...
import * as sortHelper from './helper';

export const sortTest = (n, rangeL, rangeR) => {

  const testArray = [
    { arr: sortHelper.generateRandomArray(n, rangeL, rangeR), tag: '完全随机数组' },
    { arr: sortHelper.generateNearlyOrderedArray(n, 10), tag: '近乎有序随机数组' }
  ];
  ...
  for (let i = 0; i < testArray.length; i++) {
    ...
    const arr9 = sortHelper.copyArray(arr, n);
    const arr10 = sortHelper.copyArray(arr, n);
    ...
    sortHelper.sortTime(sort.quickSortOptimize, arr9, n);
    sortHelper.sortTime(sort.quickSort3Ways, arr10, n);
  }
};
